...
const selectedUser = ref('') // Volvemos a usar String
const startDate = ref('')
const endDate = ref('')
const metrics = ref<UserMetricsItem[]>([])
const selectedPeriod = ref('week')

const getFullName = (user: staff) => {
...
async function getUsersStaff() {
  ...
  // SOLUCIÓN: Validamos que exista la posición [0] antes de intentar leerla
  if (users.value.length > 0 && selectedUser.value === '') {
    selectedUser.value = users.value[0] ? getFullName(users.value[0]) : ''
  }
}
...
async function getUserMetrics() {
  ...
  params.append('user', selectedUser.value) // Enviamos el NOMBRE
  ...
  const url = `/user/metrics?${params.toString()}`
  ...
  try {
    ...
    if (error.value) {
      errorMessage.value = error.value.message || 'Error al obtener datos'
      metrics.value = []
      return
    }
...
<template>
  <div class="w-full flex flex-col">
    ...
    <div class="bg-gray-50 border border-gray-200 p-4 sm:p-5 rounded-2xl flex flex-col sm:flex-row flex-wrap gap-4 items-end mb-6 w-full shadow-sm">
      ...
      <div class="flex flex-col gap-1.5 w-full sm:w-auto">
        ...
        <div class="relative">
          <select v-model="selectedUser" class="w-full sm:w-56 appearance-none bg-white border border-gray-300 rounded-xl px-4 py-2.5 text-sm font-medium focus:ring-2 focus:ring-[#0B369E] outline-none transition shadow-sm">
            <option v-for="user in users" :key="user.id" :value="getFullName(user)">
              {{ getFullName(user) }}
            </option>
          </select>
      ...
      <div v-if="selectedPeriod === 'custom'" class="flex flex-col gap-1.5 w-full sm:w-auto animate-in fade-in duration-200">
    ...
    <div class="w-full">
      ...
      <div v-if="isLoading" class="py-12 text-center text-blue-600 font-bold bg-white rounded-2xl border border-gray-200 shadow-sm">
        <i class="fas fa-spinner animate-spin text-2xl mb-2"></i><br>Consultando métricas...
      </div>
          ...
          <table class="w-full text-left border-collapse min-w-[1100px]">
            <thead class="bg-[#1a2b4b] text-white uppercase text-[10px] sm:text-xs font-bold tracking-wider">
              <tr>
                ...
                <th class="px-4 sm:px-5 py-3 sm:py-4 text-center whitespace-nowrap text-rose-300">Ventas Can.</th>
                <th class="px-4 sm:px-5 py-3 sm:py-4 text-center whitespace-nowrap">Pedidos</th>
                <th class="px-4 sm:px-5 py-3 sm:py-4 text-center whitespace-nowrap text-rose-300">Pedidos Can.</th>
                <th class="px-4 sm:px-5 py-3 sm:py-4 text-right whitespace-nowrap bg-blue-900/50">Ingresos Venta</th>
                <th class="px-4 sm:px-5 py-3 sm:py-4 text-right whitespace-nowrap bg-emerald-900/50">Ingresos Pedido</th>
                <th class="px-4 sm:px-5 py-3 sm:py-4 text-center whitespace-nowrap">Mov. Inv.</th>
            ...
            <tbody class="divide-y divide-gray-100 text-xs sm:text-sm">
              <tr v-for="item in metrics" :key="item.user_id" class="hover:bg-gray-50 transition-colors">
      ...
      <div v-else-if="!isLoading" class="py-16 text-center bg-white rounded-2xl border border-dashed border-gray-300 shadow-sm">
        <div class="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-3 text-gray-300">
            <i class="fas fa-user-clock text-2xl"></i>
        </div>
        <p class="text-gray-500 font-bold">Sin actividad registrada en este periodo.</p>
      </div>
    </div>
  </div>
</template>