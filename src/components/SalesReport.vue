...
async function getReport() {
  ...
  try {
    ...
    if (selectedPeriod.value === 'custom') {
      ...
      if (startDate.value > endDate.value) {
        errorMessage.value = 'Fechas inválidas'
      }
...
<template>
  <div class="w-full flex flex-col">
    ...
    <div class="bg-gray-50 border border-gray-200 p-4 sm:p-5 rounded-2xl flex flex-col sm:flex-row flex-wrap gap-4 items-end mb-6 w-full">
      ...
      <div v-if="selectedPeriod === 'custom'" class="flex flex-col gap-1.5 w-full sm:w-auto animate-in fade-in slide-in-from-left-4 duration-200">
    ...
    <div class="w-full">
      ...
      <div v-if="isLoading" class="py-12 text-center text-blue-600 font-bold bg-white rounded-2xl border border-gray-200">
        <i class="fas fa-spinner animate-spin text-2xl mb-2"></i><br>Consultando datos de ventas...
      </div>
        ...
        <div class="overflow-x-auto w-full">
          <table class="w-full text-left border-collapse min-w-[950px]">
            <thead class="bg-[#1a2b4b] text-white uppercase text-[10px] sm:text-xs font-bold tracking-wider">
              <tr>
                <th class="px-4 sm:px-5 py-3 sm:py-4 whitespace-nowrap">Código</th>
                <th class="px-4 sm:px-5 py-3 sm:py-4 whitespace-nowrap">Producto</th>
                <th class="px-4 sm:px-5 py-3 sm:py-4 text-center whitespace-nowrap">Cantidad</th>
                ...
                <th class="px-4 sm:px-5 py-3 sm:py-4 text-right whitespace-nowrap bg-blue-900/50">Ingresos</th>
                <th class="px-4 sm:px-5 py-3 sm:py-4 text-right whitespace-nowrap bg-rose-900/50">Costo</th>
                <th class="px-4 sm:px-5 py-3 sm:py-4 text-right whitespace-nowrap bg-emerald-900/50">Ganancia</th>
                <th class="px-4 sm:px-5 py-3 sm:py-4 text-center whitespace-nowrap">Margen</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 text-xs sm:text-sm">
              ...
              >
                <td class="px-4 sm:px-5 py-3 sm:py-4 font-bold text-gray-500">{{ item.codigo }}</td>
                <td class="px-4 sm:px-5 py-3 sm:py-4 font-bold text-gray-900">{{ item.name }}</td>
                <td class="px-4 sm:px-5 py-3 sm:py-4 text-center font-medium">{{ item.cantidad }}</td>
                <td class="px-4 sm:px-5 py-3 sm:py-4 text-center text-gray-600">{{ item.ventas_realizadas }}</td>
                <td class="px-4 sm:px-5 py-3 sm:py-4 text-center text-gray-600">{{ item.pedidos_realizados }}</td>
                <td class="px-4 sm:px-5 py-3 sm:py-4 text-right font-black bg-blue-50/30 text-blue-700">${{ item.ingresos }}</td>
                <td class="px-4 sm:px-5 py-3 sm:py-4 text-right font-bold bg-rose-50/30 text-rose-700">${{ item.costo }}</td>
                <td class="px-4 sm:px-5 py-3 sm:py-4 text-right font-black bg-emerald-50/30 text-emerald-700">${{ item.ganancia }}</td>
                <td class="px-4 sm:px-5 py-3 sm:py-4 text-center font-bold" :class="Number(item.margen) > 30 ? 'text-emerald-600' : 'text-amber-600'">
                  {{ item.margen }}%
                </td>
              </tr>
            </tbody>
      ...
      <div v-else-if="!isLoading" class="py-16 text-center bg-white rounded-2xl border border-dashed border-gray-300">
        ...
        <p class="text-gray-500 font-bold">No hay datos de ventas en el periodo seleccionado.</p>
